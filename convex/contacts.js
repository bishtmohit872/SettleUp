...
export const getAllContacts = query({
  handler: async (ctx) => {
    ...
    const expensesYouPaid = await ctx.db
      .query("expenses")
      .withIndex("by_user_and_group",
        (q) => q.eq("paidByUserId", currentUser._id).eq("groupId", undefined)
      ).collect();
    ...
    const contactIds = new Set(); //users which are involved in payments except current users
    personalExpenses.forEach((exp) => {
      if (exp.paidByUserId !== currentUser._id) {
        contactIds.add(exp.paidByUserId);
      }

      // user which are involved in splits except current users
      ...
      [...contactIds].map(async (id) => {
        ...
        return u
          ? {
              ...
              email: u.email,
              imageUrl: u.imageUrl,
              type: "user", //marker to differentiate from groups
            }
          : null;
      })
    );
...
export const createGroup = mutation({
  args: {
    ...
    members: v.array(v.id("users")), //group members
  },
