...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    .collect())
    .filter((e)=>e.paidByUserId===user._id || e.splits.some((s)=>s.userId===user._id))
    ...
    const settlements = (await ctx.db.query("settlements").withIndex('by_group',(q)=>q.eq('groupId',undefined)).collect()).filter((s)=>s.paidByUserId===user._id || s.receivedByUserId===user._id)
