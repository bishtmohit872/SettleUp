    ...
    handler:async(ctx)=>{
        ...
        for(const user of users){
            ...
            for(const exp of expenses){
                //if somebody else paid and user appear in split
                if(exp.paidByUserId !== user._id){
                    ...
                    const entry = ledger.get(exp.paidByUserId) ?? {amount:0,since:exp.date}
                    ...
                    //if user has paid , and other people appear in split
                    for(const s of exp.splits){
                        if(s.userId === user._id || s.paid){
                            continue
                        }
                        
                        const entry = ledger.get(s.userId) ?? {amount:0, since:exp.date}
...
//users's expenses for AI Insights
export const getUsersWithExpenses = query({
    handler:async(ctx)=>{
        ...
        for(const user of users){
            ...
            .withIndex('by_date',(q)=>q.gte('date',monthStart)).collect()
...
//specific user's expenses of past month
